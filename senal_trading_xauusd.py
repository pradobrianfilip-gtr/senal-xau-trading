...
def _obtener_velas():
    ...
    try:
        ...
        if _usando_respaldo:
            _usando_respaldo = False
            print("Volvimos a la fuente principal (Twelve Data) -- aviso interno, no se manda a Discord.")
        return velas
        ...
        try:
            ...
            if velas and not _usando_respaldo:
                ...
                print(f"Usando fuente de respaldo (Alpha Vantage) -- Twelve Data no respondio ({e}). Aviso interno, no se manda a Discord.")
            return velas
...
def revisar_senal():
    ...
    try:
        ...
        for patron in ([] if no_enviar_entradas else (_detectar_doble_techo_suelo(swings, velas) + _detectar_hch(swings, velas))):
            ...
            print(f"Patron {patron['nombre']} confirmado ({tipo_senal_patron}) -- mandando senal limpia a Discord.")
            enviar_discord(
                tipo_senal_patron,
...
def generar_reporte_bajo_demanda():
    ...
    try:
        ...
        descripcion = (
            ...
            f"**ATR (14):** {texto_atr}\n"
            f"**Veredicto del dia:** {veredicto_sesiones} "
            f"(Tokio: {detalle_sesiones.get('Tokio')} | Londres: {detalle_sesiones.get('Londres')} | "
            f"Nueva York: {detalle_sesiones.get('Nueva York')} | Sidney: {detalle_sesiones.get('Sidney')})\n"
            f"**Sesion(es) abierta(s) ahora:** {etiqueta_sesion}\n"
