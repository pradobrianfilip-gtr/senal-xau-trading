...
def enviar_discord(titulo, descripcion, color=0xF5A623):
    ...
    try:
        asyncio.run_coroutine_threadsafe(_canal.send(embed=embed), client.loop)
...
def _obtener_tendencia_diaria(velas):
    """Calcula el sesgo del dia usando las MISMAS velas de 15 minutos que ya
    tenemos (no una consulta aparte), sincronizado con la medianoche de
    Madrid -- asi el 'dia' es siempre el dia de Madrid, sin importar en que
    zona horaria trabaje la API por dentro. Devuelve 'alcista', 'bajista' o
    None si todavia no hay ninguna vela del dia de hoy."""
    ...
    for v in velas:
        try:
            momento_madrid = _parsear_timestamp_utc(v["t"]).astimezone(MADRID_TZ)
        except ValueError:
            continue
        if momento_madrid >= inicio_dia_madrid:
            velas_de_hoy.append(v)

    if not velas_de_hoy:
        return None

    apertura = velas_de_hoy[0]["o"]
    cierre = velas_de_hoy[-1]["c"]
...
def _calcular_serie_rsi(precios, periodo=PERIODO_RSI):
    """Calcula el RSI para cada vela (version simplificada, con promedio
    simple en vez del suavizado de Wilder -- mas facil de leer en el codigo,
    da valores muy similares en la practica). Devuelve una lista del mismo
    largo que 'precios', con None en las posiciones donde todavia no hay
    suficiente historial para calcularlo."""
    ...
    for i in range(periodo, len(precios)):
        ...
        ganancias = [c for c in ventana if c > 0]
        perdidas = [-c for c in ventana if c < 0]
...
def revisar_senal():
    ...
    try:
        ...
        enviar_discord(
            ...
            f"RSI de confirmacion: {rsi_actual}\n\n"
            f"{texto_fvg}\n\n"
